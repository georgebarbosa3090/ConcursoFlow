...
const bancas = ["Cebraspe", "FGV", "FCC", "VUNESP", "CESPE", "IBFC", "Quadrix", "NC-UFPR", "Instituto AOCP", "FEPESE", "Fundação Cesgranrio", "Fundação Getulio Vargas (FGV)"];

function NovoConcursoForm() {
  ...
  const [url, setUrl] = useState("https://concurso-mock.pdf");

  useEffect(() => {
    const pTitle = searchParams.get("title");
    const pBoard = searchParams.get("board");
    if (pTitle) setTitle(pTitle);
    if (pBoard) setBoard(pBoard);
  }, [searchParams]);
    ...
    <MainLayout>
      <div className="p-6 md:p-8 max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
          <Suspense fallback={<div className="flex items-center justify-center p-6"><Loader2 className="animate-spin text-blue-500" /></div>}>
            <NovoConcursoForm />
          </Suspense>
        </div>
      </div>
